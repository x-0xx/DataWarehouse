...
client = MongoClient('mongodb://182.254.242.79/')

db = client['SourceCode']

collection = db['DomesticProductTaskTable']

results = collection.find({}, {'url': 1, 'id': 1})
...
for result in results:
    ...
    class inMongo(feapder.AirSpider):
        ...
        def before_parse(self, request, response):
            dbid = request.dbid
            for urlbox in response.xpath('.//div[@class="pro_description"]'):
                _ = urlbox.xpath('.//div[@class="right_info"]')
                url = "".join(_.xpath(".//@href").getall())

                if url.find("http://www.jonln.com") == -1:
                    url = "http://www.jonln.com" + url
                print(url)
        ...
        def parse(self, request, response):
            ...
            print(request.url)
            path = {}
            ol_tag = response.xpath('//ol[@class="breadcrumb"]')
            if ol_tag:
                # 获取所有<a>标签的文本内容，去除首尾空白，并用'>>'连接
                path['path'] = '>>'.join(["".join(a.xpath(".//text()").getall()) for a in ol_tag.xpath(".//a")])
            print(path)
